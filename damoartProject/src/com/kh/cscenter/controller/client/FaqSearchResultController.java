...
/**
 * Servlet implementation class FaqSearchResultController
 */
@WebServlet("/search.fa")
public class FaqSearchResultController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FaqSearchResultController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new FaqService().selectSearchListCount(keyword); // 총 게시글 수
		...
		ArrayList<FAQ> list = new FaqService().selectSearchList(pi, keyword);
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		
		request.getRequestDispatcher("views/cscenter/client/faqSearchResultView.jsp").forward(request, response);
	}
